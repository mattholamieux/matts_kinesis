...
function sun_mode_3.init(self)
    ...
    for i = 1, #self.reflector_locations do
        ...
        self.play[rid]      = 1
        self.loop[rid]      = 1
        sun_mode_3.deselect_reflector(self, rid)
    end
...
function sun_mode_3.enc(self, n, delta)
    if n == 1 then
        ...
        if alt_key == true then
            ...
            sun_mode_3.deselect_reflector(self, self.selected_ray)
...
function sun_mode_3.select_reflector(self, reflector_id)
    ...
    local set_reflector_brightness = function(photon_id, photon)
        ...
        if photon_id < photons_per_ray then
            ...
            local brightness = sun_mode_3.get_photon_brightness(self, reflector_id, photon_id)
            -- local morphing_callback = function(next_val, done) end
            -- photon:morph_photon(15, brightness, 1, 15, 'lin', morphing_callback, reflector_id)
            photon:morph_photon(15, brightness, 1, 15, 'lin', nil, reflector_id)
            print("return nil")
            return nil
        end
    end
    ...
    -- for photon = 1, photons_per_ray do
        ...
        -- else
        --     local brightness = sun_mode_3.get_photon_brightness(self, reflector_id, photon)
...
function sun_mode_3.init_lattice(self)
    ...
    self.lattice:start()
    if sun == 1 then self.sprocket_1:start() end
    if sun == 2 then self.sprocket_2:start() end
    -- -- stop the first sprocket for sun 2
    -- if self.index == 2 then self.sprocket_1:stop() end
    -- -- stop the 2nd sprocket for both suns
    -- self.sprocket_2:stop()
...
-- init lattice
function sun_mode_3.init_sounds(self)
    sun_mode_3.init_lattice(self)
end
    ...
    if sun == 1 then
        if event_type == "process" then 
            if reflector_id == 1 then 
                sun_mode_3.save_note(self,reflector_id,value)
            elseif reflector_id == 5 then
                sun_mode_3.set_cutoff(self,reflector_id,value)
            elseif reflector_id == 9 then
                sun_mode_3.set_attack(self,reflector_id,value)
            elseif reflector_id == 13 then
                sun_mode_3.set_release(self,reflector_id,value)
            end
        end
        if event_type == "sprocket"      then sun_mode_3.play_note(self) end
    ...
    elseif sun == 2 then
        ...
        if event_type == "process" then 
            ...
            elseif reflector_id == 11 then
                sun_mode_3.set_resonance(self,reflector_id,value)
            elseif reflector_id == 15 then
                sun_mode_3.set_amp(self,reflector_id,value)
            end
        end
        if event_type == "sprocket"      then sun_mode_3.play_note(self) end
        -- unused events
        if event_type == "end_of_loop"  then 
            if reflector_id == 3 then
                print("end of loop 3 reset division") 
                local curr_div = suns[2].sprocket_2.division
                suns[2].sprocket_2.division = curr_div == 1/16 and 1/32 or 1/16
            else
                print("end of loop 7 reset swing") 
...
function sun_mode_3.save_note(self,reflector_id,value)
...
function sun_mode_3.play_note(self)
    if self.note then
...
function sun_mode_3.set_cutoff(self,reflector_id,value)
    local cutoff = math.floor(util.linlin(1,128,100,10000,value))
    engine.cutoff(cutoff)
    -- print("set_cutoff",cutoff)
end 

function sun_mode_3.set_attack(self,reflector_id,value)
    local attack = util.linlin(1,128,0.001,1,value)
    engine.attack(attack)
    -- print("attack",attack)
end 

function sun_mode_3.set_release(self,reflector_id,value)
    local release = util.linlin(1,128,0.1,1,value)
    engine.release(release)
    -- print("release",release)
end 

function sun_mode_3.set_resonance(self,reflector_id,value)
    local resonance = util.linlin(1,128,0,3,value)
    engine.resonance(resonance)
    -- print("resonance",resonance)
end 

function sun_mode_3.set_amp(self,reflector_id,value)
